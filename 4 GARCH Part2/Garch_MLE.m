...
[MLE,fval,~,~,~,hessian]=fminunc(lkh,[0.02,0.28,0.61]);
...
if (p_alpha0<=0.05)  
    count_alpha0 = count_alpha0+1;
end
if (p_alpha1<=0.05) 
    count_alpha1 = count_alpha1+1;
end
if (p_beta1<=0.05)
    count_beta1 = count_beta1+1;
end
i
end
...
Mdl = garch('Constant',0.01,'GARCH',0.3,'ARCH',0.6);
EstMdl = estimate(Mdl,transpose(Y));
